...
def activity_needs_update(existing_activity, new_activity):
    ...
    return (
        ...
        existing_props['Total Sets']['number'] != round(new_activity.get('totalSets', 0)) or
        existing_props['Total Reps']['number'] != round(new_activity.get('totalReps', 0)) or
        existing_props['Volume']['number'] != round(new_activity.get('totalVolume', 0)) or
        existing_props['VO2 Max']['number'] != round(new_activity.get('vo2MaxValue', 0))
    )
...
def create_activity(client, database_id, activity):
    ...
    properties = {
        ...
        "Total Sets": {"number": round(activity.get('totalSets', 0))},
        "Total Reps": {"number": round(activity.get('totalReps', 0))},
        "Volume": {"number": round(activity.get('totalVolume', 0))},
        "VO2 Max": {"number": round(activity.get('vo2MaxValue', 0))}
    }
    page = {
...
def update_activity(client, existing_activity, new_activity):
    ...
    properties = {
        ...
        "Total Sets": {"number": round(new_activity.get('totalSets', 0))},
        "Total Reps": {"number": round(new_activity.get('totalReps', 0))},
        "Volume": {"number": round(new_activity.get('totalVolume', 0))},
        "VO2 Max": {"number": round(new_activity.get('vo2MaxValue', 0))}
    }
    update = {
